...
async function capture(url, page, dir) {
  ...
  // Try to dismiss cookie banners (best effort)
  try {
    const selectors = [
...
async function main() {
  ...
  if (process.env.PAGES) {
    try { urls = JSON.parse(process.env.PAGES); } catch {}
  }
  if (!urls?.length && process.env.TARGET_URL) urls = [process.env.TARGET_URL];
  if (!urls?.length) throw new Error('No URLs provided via PAGES or TARGET_URL env var.');
